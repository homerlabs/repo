...
//    var kilgoServices = [HMService]()    // These are called "accessories" in the UI.
    ...
    var logitechServices = [HMService]()    // These are called "accessories" in the UI.
    ...
    func reloadData() {
        ...
        for accessory in home.accessories.filter({ $0.manufacturer == "Logitech" }) {
            accessory.delegate = HomeStore.shared
            print("reloadData-  accessory: \(accessory)")

            for service in accessory.services.filter({ $0.isUserInteractive }) {
                logitechServices.append(service)
...
/*    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
     ...
     //       accessoryDetail.service = kilgoServices[button.tag]
            accessoryDetail.service = logitechServices[button.tag]
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let accessoryCell = cell as? AccessoryCell {
            accessoryCell.service = logitechServices[indexPath.item]
            let service = logitechServices[indexPath.item]
            print("logitechServices[indexPath.item]   name: \(service.name)   serviceType: \(service.serviceType)  description: \(service.localizedDescription)")
         //   print("logitechServices[indexPath.item]   characteristics: \(service.characteristics)   localizedDescription: \(service.localizedDescription)")
